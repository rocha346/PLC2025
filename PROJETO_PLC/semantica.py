...
class Semantica:
    ...
    def visita(self,node):
        ...
        if isinstance(node, str):
            #Tenta ver se é uma variável declarada (ex: 'i')
            if node.lower() in ['true', 'false']:
                return 'BOOLEAN'
            try:
                info = self.procura(node)
                # Se encontrou, devolve o tipo da variável (ex: 'INTEGER')
                return info[1] 
            except:
                #Se não estiver na tabela, assume que é um texto literal (ex: '1' ou 'abc')
                return 'STRING'
    ...
    def visita_ASSIGN(self,node):
        ...
        if isinstance(var, str):
            info = self.procura(var)
            # ('VAR', 'INTEGER', None) ou ('PROCEDURE', None, []) ou Funcao...
            # Nao se pode associar valores a um procedure
            if info[0] == 'PROCEDURE':
                raise Exception(f"Não se pode atribuir valores a um procedure {var}")
            tipo_var = info[1]
        else:
    ...
    def visita_READLN(self,node):
        ...
        for var in lista_vars:
            if isinstance(var,str):
                info = self.procura(var)        # se existe
                #(VAR,INTEGER,None)
                if info[0] != 'VAR':
            ...
            #('ARRAY_ACESSO',nome,exp)
                ...
                raise Exception(f"READLN espera uma var")
    ...
    def visita_LENGTH(self,node):
        ...
        if isinstance(exp,str):
            # tentamos procurar se é uma VAR definida
            try:
                info = self.procura(exp)
                # (VAR,(ARRAY,(1,10),INTEGER), None)
                # (VAR,STRING_LITERAL,None)

                tipo_var = info[1]
                if isinstance(tipo_var, tuple) and tipo_var[0] == 'ARRAY': #é um array
                    return 'INTEGER'

                if tipo_var == 'STRING' or tipo_var == 'STRING':
                    return 'INTEGER'        #é um var string
                
                raise Exception(f"A função Length não aceita vars do tipo {tipo_var}")
            ...
            raise Exception(f"Length espera uma array ou strings, recebeu {tipo_exp}")
